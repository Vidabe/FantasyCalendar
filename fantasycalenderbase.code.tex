...
\cs_new:Npn \__fantasycal_base_lp_start:ww #1 year ~ divisible ~ by ~ #2;
  {
    \int_set:Nn \l__fantasycal_base_tmpa_int {#2}
    \__fantasycal_base_lp_store:nV { } \l__fantasycal_base_tmpa_int
...
\cs_new:Npn \__fantasycal_base_lp_contin:ww #1 but ~ #2 divisible ~ by ~ #3;
  {
    ...
    \__fantasycal_base_lp_store:nV {#2} \l__fantasycal_base_tmpa_int
    \peek_meaning_ignore_spaces:NTF b
      { \__fantasycal_base_lp_contin:ww }
      { \use_none_delimit_by_q_recursion_stop:w }
  }
...
\prg_new_conditional:Npnn \__fantasycal_base_lp_check_str_equ_not:n #1 { TF }
  {
    \tl_if_blank:nTF {#1}
      { \prg_return_false: }
      {
...
\cs_new:Npn \__fantasycal_base_lp_store:nn #1#2
  {
    \__fantasycal_base_lp_check_str_equ_not:nTF {#1}
      { 
...
%D = L + 28 - 31*(M/4).
%
%      \__fantasycal_set_stuff:nn { Y } {#1}
%      \__fantasycal_set_stuff:nn { C } { \_{\%Y}{100} }
%      \__fantasycal_set_stuff:nn { N } { \m{\%Y}{19} }
%      \__fantasycal_set_stuff:nn { K } { \_{\%C-17}{25}  }
%      \__fantasycal_set_stuff:nn { I } { \%C - \_{\%C}{4} - \_{\%C - \%K}{3} + 19*\%N + 15  }
%      \__fantasycal_set_stuff:nn { I } {  \m{\%I}{30}  }
%      \__fantasycal_set_stuff:nn { I } {  \%I - \_{\%I}{28}*( 1 - \_{\%I}{28}*(\_{29}{\%I+1}))*(\_{21 - \%N}{11})   }
%      \__fantasycal_set_stuff:nn { J } {  \%Y + \_{\%Y}{4} + \%I + 2 - \%C + \_{\%C}{4}  }
%      \__fantasycal_set_stuff:nn { J } {  \m{\%J}{7}}
%      \__fantasycal_set_stuff:nn { L } {  \%I - \%J }
%      \__fantasycal_set_stuff:nn { M } { 3 + \_{\%{L} + 40}{44} }
%      \__fantasycal_set_stuff:nn { D } { \%{L} + 28 - 31 *\_{\%M}{4} }
    \group_begin:
...
\cs_new:cpn { __fantasycal@shorthand@d-} 
  {
    { 
      \int_set:Nn \l__fantasycal_base_tmpc_int \l_fantasycal_base_current_day_tl
      \int_use:N \l__fantasycal_base_tmpc_int 
    }
  }
\cs_new:cpn { __fantasycal@shorthand@d= } 
  {
    {
      \int_set:Nn \l__fantasycal_base_tmpc_int \l_fantasycal_base_current_day_tl
      \ifnum\l__fantasycal_base_tmpc_int<10\relax
...
\cs_new:cpn { __fantasycal@shorthand@m- } 
  {
    {
      \int_set:Nn \l__fantasycal_base_tmpc_int \l_fantasycal_base_current_month_tl
      \int_use:N \l__fantasycal_base_tmpc_int
    }
  }
\cs_new:cpn { __fantasycal@shorthand@m= } 
