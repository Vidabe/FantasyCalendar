...
%
    ...
    % Start main loop
    %
    \let \tikznrdaysperweek \l__fantasycalendar_nr_days_per_week_int
    \loop%
    \ifnum \l_fantasycalendar_current_ordinal_int <\l__fantasycalendar_end_ordinal_plus_tl \relax%
...
\cs_new:Npn \__fantasycalendar_if_date:nTF #1#2#3
  {
    \__fantasycalendar_launch_if_date:nnn {#1} {#2} {#3}
  }

\cs_new:Npn \__fantasycalendar_if_date:nnnn #1#2#3#4{%
  ...
  \__fantasycalendar_ordinal_to_weekday:nN \l__fantasycalendar_tmpa_int
    {\l__fantasycalendar_tmpb_int}%
  \tl_set:NV \l_fantasycalendar_current_weekday_tl{\l__fantasycalendar_tmpb_int}%
  %
  \__fantasycalendar_launch_if_date:nnn{#2}{#3}{#4}%
}


\cs_new:Npn \__fantasycalendar_launch_if_date:nnn #1#2#3 {
  ...
  \pgfqkeys{/pgf}{#1}%
  \bool_if:NTF \l__fantasycalendar_date_match_bool {#2} {#3}
}
...
\cs_new:Npn \__fantasycalendar_define_month_at_half_year:NN #1#2
  {
    \int_set:Nn \l__fantasycalendar_month_at_half_year_int
      { #1 / 2 }
  ...
  /pgf/day ~ of ~ month/.cd,
    ...
    .code=
      {\ifnum#1=\l_fantasycalendar_current_day_tl\relax
        \bool_set_true:N \l__fantasycalendar_date_match_bool\fi
      }
}
\pgfkeys{
...
\pgfkeys{/pgf/end ~ of ~ month/.cd, .default=1, .code={
...
\pgfkeys{/pgf/equals/.cd,.value ~ required,.code={%
...
\pgfkeys{/pgf/at ~ least/.cd,.value ~ required,.code={%
  \__fantasycalendar_special_date_to_ordinal:n {#1}%
  \ifnum \l_fantasycalendar_current_ordinal_int<\l__fantasycalendar_tmpa_int\relax%
  \else%
    \bool_set_true:N \l__fantasycalendar_date_match_bool%
...
%
\pgfkeys{/pgf/at ~ most/.cd,.value ~ required,.code={%
  \__fantasycalendar_special_date_to_ordinal:n {#1}%
  \ifnum\l_fantasycalendar_current_ordinal_int>\l__fantasycalendar_tmpa_int\relax%
  \else%
    \bool_set_true:N \l__fantasycalendar_date_match_bool%
...
%
\pgfkeys{/pgf/between/.cd,.value ~ required,.code ~ args={#1and#2}{%
  \__fantasycalendar_special_date_to_ordinal:n {#1}%
  \ifnum\l_fantasycalendar_current_ordinal_int<\l__fantasycalendar_tmpa_int\relax%
  \else%
    ...
    \int_compare:nNnTF { \l__fantasycalendar_day_int } = { 0 }
      {
        ...
        \int_set:Nn \l__fantasycalendar_day_int 
          { \seq_item:Nn \l__fantasycalendar_number_days_of_months_seq { \l__fantasycalendar_month_int } }
      }{
    %% get month and day
...
\cs_new:Npn \__fantasycalendar_ordinal_to_date_get_month_aux_i:
  {
    \seq_map_indexed_inline:Nn \l__fantasycalendar_cumulative_number_days_seq
      {
        % <
        \int_compare:nNnF { \l__fantasycalendar_day_int } > {##2}
          { 
            \seq_map_break:n
              {
                \int_set:Nn \l__fantasycalendar_month_int {##1-1} 
                \int_sub:Nn \l__fantasycalendar_day_int 
                  { \__fantasycalendar_print_cumulative_days:n {##1-1} }
              }
          }
      }
  }
\cs_new:Npn \__fantasycalendar_ordinal_to_date_get_month_aux_ii:
  {
    \seq_map_indexed_inline:Nn \l__fantasycalendar_cumulative_number_days_invers_seq
      {
        % <
        \int_compare:nNnT { \l__fantasycalendar_day_int } > {##2}
          { 
            \seq_map_break:n
              {
                \int_set:Nn \l__fantasycalendar_month_int 
                  {  \l__fantasycalendar_nr_months_per_year_int - ##1 + 1 } 
                \int_sub:Nn \l__fantasycalendar_day_int {##2}
              }
          }
      }
  }
