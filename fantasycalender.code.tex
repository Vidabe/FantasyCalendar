...
\tl_set:Nn \l__fantasycal_code_day_tl 
  {
    \node
      ( \__fantasycal_base_suggested_name: )
...
\int_new:N \l__fantasycal_task_ordinal_start_int
\int_new:N \l__fantasycal_task_ordinal_end_int
\int_new:N \l__fantasycal_task_nr_tasks_today_int
\tl_new:N \l__fantasycal_task_options_tl
\tl_new:N \l__fantasycal_task_text_tl
\tl_new:N \l__fantasycal_task_colour_tl
\tl_new:N \l__fantasycal_task_linewidth_tl
\tl_new:N \l__fantasycal_task_priority_tl

\dim_new:N \l__fantasycal_task_xshift_dim
\dim_set:Nn \l__fantasycal_task_xshift_dim { -5pt }
\dim_new:N \l__fantasycal_task_yshift_dim
\dim_set:Nn \l__fantasycal_task_yshift_dim { 5pt }
\dim_new:N \l__fantasycal_task_base_y_dim
\dim_set:Nn \l__fantasycal_task_base_y_dim { 2pt }
...
\keys_define:nn { fantasycal/task }
  {
    color .tl_set:N = \l__fantasycal_task_colour_tl ,
    linewidth .tl_set:N = \l__fantasycal_task_linewidth_tl ,
    done .code:n = { \tl_set:Nn \l__fantasycal_task_priority_tl { done } } ,
    priority .choices:xn = 
      { \l__fantasycal_task_priorities_clist , done }
      {
        ...
        \int_case:nn { \l_keys_choice_int }
          {
            { 1 } { \keys_set:nn { fantasycal/task } { color=red, linewidth=ultra ~ thick } }
            { 2 } { \keys_set:nn { fantasycal/task } { color=orange, linewidth= very ~  thick } }
            { 3 } { \keys_set:nn { fantasycal/task } { color=blue, linewidth = semithick } }
            { 4 } { \keys_set:nn { fantasycal/task } { color=black, linewidth=thin } }
          }
      }
...
\NewDocumentCommand \FantasyCalendarAddTask { m m m m }
  {
    \__fantasycal_task_setup:nnnn {#1} {#2} {#3} {#4}
    \__fantasycal_task_add:VVVV
      \l__fantasycal_task_ordinal_start_int
      \l__fantasycal_task_ordinal_end_int
      \l__fantasycal_task_text_tl
      \l__fantasycal_task_options_tl
  }

\cs_new:Npn \__fantasycal_task_setup:nnnn #1#2#3#4
  {
    \__fantasycal_base_iso_to_ordinal:nN {#1} \l__fantasycal_task_ordinal_start_int
    \__fantasycal_base_iso_to_ordinal:nN {#2} \l__fantasycal_task_ordinal_end_int
    \tl_set:Nn \l__fantasycal_task_options_tl {#3}
    \tl_set:Nn \l__fantasycal_task_text_tl {#4}
  }

\cs_new:Npn \__fantasycal_task_add:nnnn #1#2#3#4
  {
    \tl_put_right:Nn \l__fantasycal_code_task_tl
      {
        \ifdateT { ordinal ~ equals = #2 }
          { 
            \int_incr:N \l__fantasycal_task_nr_tasks_today_int
            \__fantasycal_task_finish_todays_tasks:nn {#3} {#4}
          }
      }
  }
\cs_generate_variant:Nn \__fantasycal_task_add:nnnn { VVVV }

\cs_new:Npn \__fantasycal_task_finish_todays_tasks:nn #1#2
  {
    \group_begin:
      \keys_set:nn { fantasycal/task } {#2}
      \dim_add:Nn \l__fantasycal_task_base_y_dim 
        { 
          \l__fantasycal_box_inner_y_sep_dim + 
          \l__fantasycal_box_outer_y_sep_dim 
        }
      \dim_set:Nn \l_tmpa_dim 
        {
          \l__fantasycal_task_base_y_dim + 
          \baselineskip*( \l__fantasycal_task_nr_tasks_today_int - 1 )
        }
      \draw
        [
          , transform ~ canvas = 
              { xshift=\l__fantasycal_task_nr_tasks_today_int\l__fantasycal_task_xshift_dim } 
          , \l__fantasycal_task_colour_tl 
          , \l__fantasycal_task_linewidth_tl 
        ]
        (today-box.south ~ east) ++ (-3pt,\l_tmpa_dim) 
         -- ++(3pt,0) 
        node [anchor=base ~ east] {#1}
        -- (today-box.north ~ east)
        ;
      \group_end:
  }
...
\bool_new:N \l__fantasycal_print_newpage_bool
\bool_set_true:N \l__fantasycal_print_newpage_bool
...
\keys_define:nn { fantasycal/print-calendar }
  {
    ...
    ,
    newpage .bool_set:N = \l__fantasycal_print_newpage_bool
  }
    ...
    \int_step_inline:nnn 
      ...
      { \l__fantasycal_months_to_print_end_int }
      {
        ...
        \PrintFantasycalendarBase
          ...
          ;
        \end{tikzpicture}
        \bool_if:NT \l__fantasycal_print_newpage_bool
          { \newpage }
      }
  }
