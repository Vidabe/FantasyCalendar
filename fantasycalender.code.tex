...
\tl_new:N \FantasycalendarTextDay
  ...
  { 
    ...
    box ~ width/.code = 
      { 
        ...
        \tikzset 
          { 
            days = { inner~xsep = \l__fantasycalendar_box_inner_x_sep_dim }  ,
            box = { minimum~width=\l__fantasycalendar_calendar_box_width_tl } ,  
          }
      } ,
    box ~ height/.code = 
      { 
        ...
        \tikzset 
          { 
            days = { inner~ysep=\l__fantasycalendar_box_inner_y_sep_dim } ,
            box = { minimum~height=\l__fantasycalendar_calendar_box_height_tl } ,
          }
      } ,
    box ~ size/.code = 
      { 
...
\tl_new:N \l__fantasycalendar_text_day_moon_hook_tl
\tl_new:N \l__fantasycalendar_text_day_note_hook_tl
\tl_new:N \l__fantasycalendar_text_month_year_hook_left_tl
\tl_new:N \l__fantasycalendar_text_month_year_hook_right_tl

\tl_set:Nn \l__fantasycalendar_text_box_tl { \phantom{1} } %% text in node shifts thing up
\tl_set:Nn \FantasycalendarTextDay 
  { 
    \%d- 
    \l__fantasycalendar_text_day_moon_hook_tl 
    \l__fantasycalendar_text_day_note_hook_tl
  }
\tl_set:Nn \l__fantasycalendar_text_week_day_tl { \%wt }
\tl_set:Nn \l__fantasycalendar_text_month_tl 
  { 
    \bfseries 
    \l__fantasycalendar_text_month_year_hook_left_tl 
    \%mt \space [\%m0] 
    \l__fantasycalendar_text_month_year_hook_right_tl 
  }
...
\tikzset
  { 
    day ~ text/.code= { \tl_set:Nn \FantasycalendarTextDay {#1} }
  }
\tikzset
  { 
    weekday ~ text/.code= { \tl_set:Nn \l__fantasycalendar_text_week_day_tl {#1} }
  }
\tikzset
  { 
...
\tikzset
  {
    ...
    , add ~ year ~ to ~ month/left /.code = 
      { 
        \tl_clear:N \l__fantasycalendar_text_month_year_hook_right_tl
        \tl_set:Nn \l__fantasycalendar_text_month_year_hook_left_tl 
          { \l__fantasycalendar_text_year_tl \space -- \space}
      }
    , add ~ year ~ to ~ month/right /.code = 
      { 
        \tl_clear:N \l__fantasycalendar_text_month_year_hook_left_tl
        \tl_set:Nn \l__fantasycalendar_text_month_year_hook_right_tl 
          { \space -- \space \l__fantasycalendar_text_year_tl}
      }
...
\tikzset
  {
    ...
    , add ~ moons / true / .code =
       {
         \tl_set:Nn \l__fantasycalendar_text_day_moon_hook_tl 
           { \, \FantasycalendarPrintMoons }
       }
    , add ~ moons / false / .code =
       { \tl_clear:N \l__fantasycalendar_text_day_moon_hook_tl }
    , add ~ moons / .default = true
  }

\cs_new:Npn \FantasycalendarPrintMoons
  {
...
\NewDocumentCommand \FantasyCalendarSpecialEntry { m m }
  {
    \tikzset
      {
        execute ~ at ~ begin ~ day ~ scope = 
          { \ifdateT{#1}{ \__fantasycalendar_add_yearly_day:nn { \%md } {#2} } }
      }
%    \__fantasycalendar_add_yearly_day:nn {#1} {#2}
  }
...
\tikzset
  {
    ...
    , add ~ day ~ notes / true / .code =
       {
         \tl_set:Nn \l__fantasycalendar_text_day_note_hook_tl \FantasycalendarPrinDayNotes
       }
    , add ~ day ~ notes / false / .code =
       { \tl_clear:N \l__fantasycalendar_text_day_note_hook_tl }
    , add ~ day ~ notes / .default = true
  }
\cs_new:Npn \FantasycalendarPrinDayNotes
  {
    \small
