...
\cs_new:Npn \__fantasycal_dates_parse:ww #1 to #2 \q_no_value
  {
    \tl_set:Nn \l__fantasycal_dates_start_tl {#1}
    \tl_set:Nn \l__fantasycal_dates_end_tl {#2}
  }
            ...
            \str_if_eq:nnTF {#1} { fit }
              {
                ...
                \tl_set:NV \l__fantasycal_calendar_box_width_tl \l_tmpa_dim
              }{
            ...
            \str_if_eq:nnTF {#1} { fit }
              {
                ...
                \int_set:Nn \l_tmpa_int
                  { \fp_eval:n { max ( \seq_use:Nn \l_tmpa_seq { , } ) } }
                ...
                %% \l_tmpa_int is now the number of rows i have.
                \int_set:Nn \l_tmpa_int
                  { 
                    \fp_eval:n
                      {
                        ceil
                          (
                            (\l_tmpa_int + \l__fantasycal_base_nr_days_per_week_int-1) /
                ...
                \bool_if:NTF \l__fantasycal_draw_monthname_bool
                  { \dim_set:Nn \l_tmpa_dim { 3.25\baselineskip } }
                  {
                    \bool_if:NTF \l__fantasycal_draw_weekday_bool
                      { \dim_set:Nn \l_tmpa_dim { 2\baselineskip } }
                      { \dim_zero:N \l_tmpa_dim }
                  }

                \dim_set:Nn \l_tmpa_dim { ( \textheight - \l_tmpa_dim ) / \l_tmpa_int }
                \tl_set:NV \l__fantasycal_calendar_box_height_tl \l_tmpa_dim
...
\tl_set:Nn \l__fantasycal_code_day_tl 
  {
    \node
      ...
      { 
        \dim_compare:nNnTF 
          { \l__fantasycal_box_width_dim + \l__fantasycal_box_height_dim  } = { 0pt }
...
\tl_set:Nn \l__fantasycal_code_box_tl 
  { 
    \bool_lazy_and:nnF
      { \tl_if_empty_p:N \l__fantasycal_calendar_box_height_tl }
      { \tl_if_empty_p:N \l__fantasycal_calendar_box_width_tl }
      {
...
\tikzset
  { 
    ...
    print-monthnames/false/.code= 
      { \bool_set_false:N \l__fantasycal_draw_monthname_bool }  ,
        ...
        execute ~ after ~ day ~ scope=
          {
            \ifdateT {end ~ of ~ week}
              {
                \pgfmathsetlength{\l_tmpb_dim}{\l__fantasycal_draw_day_yshift_tl}
        ...
        execute ~ before ~ day ~ scope=
          {
            \ifdateT {day ~ of ~ month=1}
              {
                \ifdateF { equals = \l__fantasycal_dates_start_tl }
                  {
                    % On first of month, except when first date in calendar.
                    \pgfmathsetlength{\l_tmpa_dim}{\l__fantasycal_draw_month_yshift_tl}
            ...
            \ifdateT {day ~ of ~ month=1}
              {
                \__fantasycal_sanitize_weekday_for_tikz:N \l_tmpa_int
                \tl_set:NV \l__fantasycal_month_list_tl \l_tmpa_int
              }  
            \ifdateT {equals=\l__fantasycal_dates_start_tl}
              {
                {
                  \int_set_eq:NN \l_tmpa_int \l_fantasycal_base_current_ordinal_int
                  \int_sub:Nn \l_tmpa_int \l_fantasycal_base_current_day_tl
                  \int_incr:N \l_tmpa_int
                  \__fantasycal_base_ordinal_to_weekday:nN 
                    { \l_tmpa_int } \l_tmpb_int
                  \__fantasycal_sanitize_weekday_for_tikz:N \l_tmpb_int
                  \xdef \pgf@temp { \int_use:N \l_tmpb_int }
                }
                \tl_set_eq:NN \l__fantasycal_month_list_tl \pgf@temp
              }
          }
          ,
        execute ~ at ~ begin ~ day ~ scope=
          {
            \pgfmathsetlength \l_tmpa_dim { \l__fantasycal_draw_day_xshift_tl }
            \int_set:Nn \l_tmpb_dim { \l_fantasycal_base_current_day_tl \l_tmpa_dim }
            \dim_add:Nn \l_tmpb_dim { \l__fantasycal_month_list_tl \l_tmpa_dim }
            ...
            \ifdateT{day ~ of ~ month=1}
              {
                {
                  \pgfmathsetlength{\l_tmpa_dim}{\l__fantasycal_draw_day_xshift_tl}
                  \pgftransformxshift{\l__fantasycal_draw_width_tl \l_tmpa_dim}
                  \pgftransformxshift{-\l_tmpa_dim}
...
\cs_new:Npn \__fantasycal_add_yearly_day_parse:w #1-#2\relax
  {
    \tl_set:Nx \l_tmpa_tl { \int_eval:n {#1} - \int_eval:n {#2} }
  }
\cs_new:Npn \__fantasycal_add_one_year_day_parse:w #1-#2-#3\relax
  {
    \tl_set:Nx \l_tmpa_tl { \int_eval:n {#1} - \int_eval:n {#2} - \int_eval:n {#3} }
  }
...
\cs_new:Npn \__fantasycal_add_yearly_day:nn #1#2
  {
    ...
    \exp_after:wN 
    \__fantasycal_add_yearly_day_parse:w \l_tmpa_tl \relax
    \tl_if_exist:cTF { l__fantasycal_yearly_day_ \l_tmpa_tl _tl }
      { \tl_put_right:cn { l__fantasycal_yearly_day_ \l_tmpa_tl _tl } { \\ #2 } }
      { 
        \tl_new:c { l__fantasycal_yearly_day_ \l_tmpa_tl _tl }
        \tl_set:cn { l__fantasycal_yearly_day_ \l_tmpa_tl _tl } {#2}
      }
  }
\cs_new:Npn \__fantasycal_add_one_year_day:nn #1#2
  {
    \tl_set:Nx \l_tmpa_tl {#1}
    \exp_after:wN
    \__fantasycal_add_one_year_day_parse:w \l_tmpa_tl \relax
    \tl_if_exist:cTF { l__fantasycal_one_year_day_ \l_tmpa_tl _tl }
      { \tl_put_right:cn { l__fantasycal_one_year_day_ \l_tmpa_tl _tl } { \\ #2 } }
      { 
        \tl_new:c { l__fantasycal_one_year_day_ \l_tmpa_tl _tl }
        \tl_set:cn { l__fantasycal_one_year_day_ \l_tmpa_tl _tl } {#2}
      }
  }
