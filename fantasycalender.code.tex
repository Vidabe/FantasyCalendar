...
\tl_new:N \l__fantasycal_after_parsing_hook_tl
...
\cs_new:Npn \__fantasycal_box_size_calculate_height:N #1
  {
    ...
    \bool_if:NT \l__fantasycal_draw_monthname_bool
      {
        %% +7pt to fit on page
...
%
%                %% Now we calc the number of rows we have. 
%                \bool_if:NF \l__fantasycal_base_mod_all_zero_bool
%                  {
...
\tl_new:N \l__fantasycal_labeling_hook_tl
...
\cs_new:Npn \__fantasycal_do_once_at_begin:
  {
    \hbox_set:Nn \l_tmpa_box
      {
        \pgfinterruptpicture 
          \l__fantasycal_font_month_tl 
          \l__fantasycal_text_month_tl
        \endpgfinterruptpicture
      }
    \dim_set:Nn \l__fantasycal_month_text_width_dim 
      { \box_wd:N \l_tmpa_box }
    \dim_set:Nn \l__fantasycal_month_text_height_dim 
      { \box_ht_plus_dp:N \l_tmpa_box }  
    \hbox_set:Nn \l_tmpa_box
      {
        \pgfinterruptpicture 
        \l__fantasycal_font_week_day_tl
        \seq_use:Nn \l__fantasycal_base_day_names_seq { }
        \endpgfinterruptpicture
      }
    \dim_set:Nn \l__fantasycal_week_text_height_dim
      { \box_ht_plus_dp:N \l_tmpa_box }
...
\cs_new:Npn \__fantasycal_weekday_plus_month_vspace:
  {
    \__fantasycal_weekday_vspace: +
    \__fantasycal_month_vspace:
  }
...
\tl_set:Nn \l__fantasycal_code_day_tl 
  {
    \node
      ( \__fantasycal_base_suggested_name: ) (today) [ every ~ day  ]
      { 
...
%\tl_put_right:Nn \l__fantasycal_before_day_tl
%  {
%    \ifdateT { equals = \l__fantasycal_dates_start_tl }
%      {
%        {
%          \pgfmathsetlength \l__fantasycal_tmpb_dim { \l__fantasycal_draw_day_xshift_tl}
%          \__fantasycal_sanitize_weekday_for_tikz:N \l__fantasycal_tmpa_int
%          \dim_set:Nn \l__fantasycal_tmpa_dim  { \l__fantasycal_tmpb_dim }
%          \int_step_inline:nnn { 1 } { \l__fantasycal_tmpa_int-1 }
%            {
%              \pgftransformxshift { \l__fantasycal_tmpa_dim }
%              \node 
%                [ every ~ box , dashed ] 
...
%      }
%  }
%\tl_put_right:Nn \l__fantasycal_after_day_tl
%  {
%    \ifdateT { equals = \l__fantasycal_dates_end_tl }
%      {
%        {
...
%          \dim_set:Nn \l__fantasycal_tmpa_dim  { \l__fantasycal_tmpb_dim }
%          \pgftransformxshift { \l__fantasycal_tmpa_int\l__fantasycal_tmpa_dim }
%          \int_step_inline:nnn { \l__fantasycal_tmpa_int+1 } { \l__fantasycal_base_nr_days_per_week_int }
%            {
%              \pgftransformxshift { \l__fantasycal_tmpa_dim }
%              \node 
%                [ every ~ box , dashed ] 
...
\tikzset
  {
    week ~ list/.style = 
      {
        execute  ~ now = { \bool_set_true:N \l__fantasycal_week_list_bool \tl_set:Nn \l__fantasycal_list_type_tl { week-list } } ,
        execute ~ before ~ day ~ scope =
          {
...
\tikzset
  {
    month ~ label ~ left ~ vertical/.style=
      {
        __label_month_print = { left-vertical } ,
        execute ~ before ~ day ~ scope=
          {
            \ifdateT {day ~ of ~ month=1} { \l__fantasycal_code_month_tl } 
          } ,
...
\tikzset
  {
    month ~ label ~ right ~ vertical/.style=
      {
        __label_month_print = { right-vertical } ,
        execute ~ before ~ day ~ scope=
          {
            \ifdateT{day ~ of ~ month=1}
              {
