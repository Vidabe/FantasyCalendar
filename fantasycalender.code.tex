...
\cs_new:Npn \__fantasycal_sanitize_weekday_for_tikz:N #1
  {
    \int_compare:nNnTF { \l_fantasycal_base_current_weekday_tl } = 0
        ...
        \str_if_eq:nnTF {#1} { fit }
          {
            ...
            \dim_set:Nn \l_tmpa_dim
              { \textwidth / \l__fantasycal_base_nr_days_per_week_int }
        ...
        \str_if_eq:nnTF {#1} { fit }
          {
            ...
            \fp_set:Nn \l_tmpb_fp
              { 
                ( \l_tmpa_fp + \l__fantasycal_base_nr_days_per_week_int - 1 ) /
                \l__fantasycal_base_nr_days_per_week_int 
              }
            \dim_set:Nn \l_tmpa_dim {  \textheight / \fp_eval:n { floor(\l_tmpb_fp) } }
...
\tl_set:Nn \l__fantasycal_code_week_day_tl 
  { \node [every ~ weekday] { \l__fantasycal_text_week_day_tl } ; }
\tl_set:Nn \l__fantasycal_code_month_tl 
  { \node [every ~ month] { \l__fantasycal_text_month_tl } ; }
\tl_set:Nn \l__fantasycal_code_year_tl 
  { \node [every ~ year] { \l__fantasycal_text_year_tl } ; }
...
\tikzset
  { 
    day ~ text/.code= { \tl_set:Nn \fantasycalTextDay {#1} }
  }
\tikzset
  { 
...
\cs_new:Npn \__fantasycal_add_yearly_day:nn #1#2
  {
    \tl_if_exist:cTF { l__fantasycal_yearly_day_ #1 _tl }
      { \tl_put_right:cn { l__fantasycal_yearly_day_ #1 _tl } { \\ #2 } }
      { 
        \tl_new:c { l__fantasycal_yearly_day_ #1 _tl }
        \tl_set:cn { l__fantasycal_yearly_day_ #1 _tl } {#2}
      }
  }
\cs_new:Npn \__fantasycal_add_one_year_day:nn #1#2
  {
    \tl_if_exist:cTF { l__fantasycal_one_year_day_ #1 _tl }
      { \tl_put_right:cn { l__fantasycal_one_year_day_ #1 _tl } { \\ #2 } }
      { 
        \tl_new:c { l__fantasycal_one_year_day_ #1 _tl }
        \tl_set:cn { l__fantasycal_one_year_day_ #1 _tl } {#2}
      }
  }
...
\NewDocumentCommand \FantasyCalendarSpecialEntry { m m }
  {
    \tikzset
      {
        execute ~ at ~ begin ~ day ~ scope = 
          { \ifdateT{#1}{ \__fantasycal_add_yearly_day:nn { \%md } {#2} } }
      }
%    \__fantasycal_add_yearly_day:nn {#1} {#2}
  }
